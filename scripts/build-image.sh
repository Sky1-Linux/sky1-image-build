...
mkfs.ext4 -L sky1root -q "$ROOT_PART"
...
# Find kernel version
KERNEL_VERSION=$(ls "$MOUNT_DIR/boot/vmlinuz-"*-sky1 2>/dev/null | head -1 | sed 's|.*/vmlinuz-||')
if [ -z "$KERNEL_VERSION" ]; then
    KERNEL_VERSION=$(ls "$MOUNT_DIR/boot/vmlinuz-"* | head -1 | sed 's|.*/vmlinuz-||')
fi
