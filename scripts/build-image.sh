...
# ACPI boot: no acpi=off, no devicetree — firmware provides ACPI tables
# ttyAMA0 (not ttyAMA2) because ACPI SPCR enumerates the debug UART as ttyAMA0
CMDLINE_ACPI="loglevel=7 console=tty0 console=ttyAMA0,115200"
CMDLINE_ACPI+=" efi=noruntime earlycon=efifb earlycon=pl011,0x040d0000"
CMDLINE_ACPI+=" clk_ignore_unused linlon_dp.enable_fb=1 linlon_dp.enable_render=0"
CMDLINE_ACPI+=" fbcon=map:01111111 keep_bootcon panic=30"
CMDLINE_ACPI+=" root=UUID=$ROOT_UUID rootwait rw"
...
menuentry 'Sky1 Linux ${KERNEL_VERSION} - ACPI: All CIX P1 Systems' {
    linux (\$root)/boot/vmlinuz-$KERNEL_VERSION \\
        $CMDLINE_ACPI
    initrd (\$root)/boot/initrd.img-$KERNEL_VERSION
}
