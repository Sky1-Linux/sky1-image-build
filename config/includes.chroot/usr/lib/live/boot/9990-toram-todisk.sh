	...
	then
		...
		then
			...
			then
				...
				rsync -a --progress ${MODULETORAMFILE} ${copyto} 1>/dev/console
				echo " * Copy to RAM complete" 1>/dev/console
			else
				cp ${MODULETORAMFILE} ${copyto}
			fi
		else
			if [ -x /bin/rsync ]
			then
				echo " * Copying whole medium to RAM" 1>/dev/console
				rsync -a --progress ${copyfrom}/* ${copyto} 1>/dev/console
				echo " * Copy to RAM complete" 1>/dev/console
			else
